...
void setup()
{
    ...
    float d, dx, dy, dz;
    int test;
    /*This changes how many threads are on each block*/
	int blockSize = 256;
    BlockSize.x = blockSize;
	BlockSize.y = 1;
	BlockSize.z = 1;
	/*This allows us to use multiple blocks*/
	GridSize.x = (N + blockSize - 1) / blockSize;
	GridSize.y = 1;
	GridSize.z = 1;
...
__global__ void leapFrog(float3 *p, float3 *v, float3 *f, float *m, float g, float h, float damp, float dt, float t, int n)
{
	float dx, dy, dz,d,d2;
	float force_mag;
	/*This makes it to where each thread is using a different body*/
	int i = blockIdx.x * blockDim.x + threadIdx.x;
	/*This was a bounds check to make sure we weren't going outside of our bounds. This makes sure threads outside of our bounds do nothing*/
	if(i >= n)
	{
		return;
	}
	...
	//Took both sync threads out since we are using multiple blocks in threads in different blocks can't sync.
	//__syncthreads();
